...
func (lri *LineReadingInput) validateInputFile(inputFilePath string) error {
	// exists
	...
	if inputFileInfo.IsDir() {
		return fmt.Errorf("input file is a directory, not a file")
	}
	...
	if filepath.Ext(inputFilePath) != ".json" && filepath.Ext(inputFilePath) != ".yaml" && filepath.Ext(inputFilePath) != ".yml" {
		return fmt.Errorf("input file is not a json or yaml file")
	}
	...
	if !result.Valid() {
		...
		return fmt.Errorf("file does not match schema")
	}
...
func (lri *LineReadingInput) AddInputFile(inputFilePath string) error {
	...
	if err := lri.validateInputFile(absInputFilePath); err != nil {
		return err
	}

	lri.files = append(lri.files, absInputFilePath)
	return nil
}
...
func (lro LineReadingOutput) Path() (string, error) {
	absPath, err := filepath.Abs(lro.path)
	if err != nil {
		return "", err
	}
	return absPath, nil
...
var (
	LineReadingSchemaJson string
	lineReadingOutput     LineReadingOutput
	lineReadingInput      LineReadingInput
)
...
var lineRecordingCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Println("[error] invalid output path:", lineReadingOutput.path)
			fmt.Println(err)
			os.Exit(1)
			return
		}
		...
		if len(lineReadingInput.files) == 0 {
			fmt.Println("[error] no input files")
			os.Exit(1)
			return
		}

		for _, inputFile := range lineReadingInput.files {
...
func init() {
	lineRecordingCmd.Flags().StringVarP(&lineReadingOutput.path, "output", "o", defaultOutputDir, fmt.Sprintf("Directory to output files to (default: %s)", defaultOutputDir))
}
